...
if check_password():
    st.title("FCC内部用 DALL-E3 画像生成 v.0.1")
    ...
    # Add custom CSS to set button width
    st.markdown(f"""
    <style>
    .stButton > button {{
        width: {button_width};
    ...
    # Display buttons with custom width
    st.button("送信", key="submit")
    st.button("リセット", key="reset")

    if st.session_state.get('submit') and user_input:
        with st.spinner("画像生成中..."):
            try:
                # Combine all previous prompts with new user input
                combined_prompt = ' '.join(chat['prompt'] for chat in st.session_state['chat_history']) + ' ' + user_input
                
                # Add "日本人の" or "日本の" to the prompt
